...
export default class PortalEditor {
    constructor() {
        ...
        addStyle("portaleditor/portaleditor");

        const tie = new Laces.Tie(
            this.portal,
            `<div class="portal-editor dialog" style="display: none">
                <div class="column">
                    <p>
    ...
    edit(portal, options) {
        ...
        this.portal.set(
            "direction",
            isDirection(portal.name) ? portal.name : ""
        );
        ...
        $(".visualEventMultiplier", this.element).val(
            100 * (multipliers["Visual"] || 1.0)
        );
        $(".soundEventMultiplier", this.element).val(
            100 * (multipliers["Sound"] || 1.0)
        );
    ...
    setDirection(direction) {
        this.portal.direction = isDirection(direction) ? direction : "";

        if (!this.portal.name || isDirection(this.portal.name)) {
            this.portal.name = direction;
        }

        if (!this.portal.name2 || isDirection(this.portal.name2)) {
            this.portal.name2 = opposingDirection(direction);
        }
    }
    ...
    save() {
        ...
        if (this.portal.room2.id) {
            ...
            if (isDirection(this.portal.direction)) {
                var distance = parseInt(
                    $("input.distance", this.element).val()
                );
                if (distance === 0) {
                ...
                var vector = vectorForDirection(this.portal.direction);
                portal.position = [
                    this.portal.room.x + distance * vector[0],
        ...
        portal.eventMultipliers = {
            Visual:
                parseFloat($(".visualEventMultiplier", this.element).val()) /
                100,
            Sound:
                parseFloat($(".soundEventMultiplier", this.element).val()) /
                100,
        };
